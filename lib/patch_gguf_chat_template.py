...
def skip_value(file: BinaryIO, value_type: int) -> None:
    ...
    if value_type in FIXED_SIZE:
        file.seek(FIXED_SIZE[value_type], 1)
    elif value_type == TYPE_STRING:
        (length,) = struct.unpack("<Q", file.read(8))
        file.seek(length, 1)
    elif value_type == TYPE_ARRAY:
        element_type, count = struct.unpack("<IQ", file.read(12))
        if element_type in FIXED_SIZE:
            file.seek(FIXED_SIZE[element_type] * count, 1)
...
def find_chat_template(file: BinaryIO) -> tuple[int, int]:
    ...
    kv_count = struct.unpack("<QQ", file.read(16))[1]

    for _ in range(kv_count):
        key = read_string(file)
        (value_type,) = struct.unpack("<I", file.read(4))
