# SeedVR2のCLIをサブプロセスとして起動して長尺動画をアップスケールするノード。
# 公式ノードは動画全体を非圧縮テンソルとしてRAMへ一括展開するため、
# 長尺動画ではOOM Killerに殺される。
# CLIはチャンク単位で処理して境界へ時間的文脈を渡しつつ逐次エンコードするので、
# RAM使用量がチャンク数へ依存せず長尺でも完走できる。
#
# ローダノードのSEEDVR2_DIT/SEEDVR2_VAE辞書はCLI引数へ変換して渡す。
# CLI側に対応する引数がない設定(VAE選択・torch compile・デバイス分離)は、
# 黙って無視せずValueErrorで失敗させる方針。
#
...
# ワークフロー側で元動画の比率から出力解像度を自動計算するために使う。
import logging
import os
import signal
import subprocess
