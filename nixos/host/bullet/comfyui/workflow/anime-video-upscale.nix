...
# 幅と高さは偶数へ丸める。
# 既定のgbrp10le(RGB 4:4:4)はサブサンプリングしないので偶数の必要はないが、
# losslessを無効にした時のyuv420p10le(4:2:0)フォールバックと互換にするため。
#
...
#
# SeedVR2 CLIは動画をチャンク単位(chunk_sizeウィジェット、既定128フレーム)で処理し、
# チャンク境界へ時間的文脈を渡す。
