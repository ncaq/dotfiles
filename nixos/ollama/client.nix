...
let
  ...
  ollama-client = pkgs.writeShellApplication {
    ...
    # モデルの生成長の上限になる。
    # Claude Codeがモデル名から上限を引けない場合の既定は32000である。
    #
    # ここで設定する狙いは、
    # 既定が小さすぎるから広げることではなく、
    # 圧縮の閾値と足してcontextに収まる配分にすることである。
    # 応答が途中で切れる問題の主因はチャットテンプレートの側にあり、
    # 未知のreasoning effortをxhighへ倒して思考を最大まで伸ばしていたためだった。
    # そちらは`nixos/ollama/qwen3.8-chat-template.patch`でmediumへ倒して直してある。
    #
    # llama.cppのcontextはプロンプトと生成したトークンの両方を含むため、
    # 圧縮の閾値をcontextそのものにすると、
    # 長い会話で長い応答を生成した瞬間に合計が溢れる。
    # 配分は`lib/ollama-context.nix`が決めていて、
    # CPUのホストでは応答の上限が既定の32000を下回るが、
    # それは短いcontextで会話の長さを優先した結果である。
    #
    # Claude Codeはモデル名を引けない場合、
    # 圧縮の閾値をモデルのcontextで、
    # 応答の上限を128000で丸める。
    # CUDAのホストの98304と32768も、
    # CPUのホストの24576と8192も、
    # どちらの丸めにも掛からずそのまま通る。
    #
